...
namespace Internal.Cryptography.Pal.Native
{
    ...
    internal sealed class SafeCertContextHandleWithKeyContainerDeletion : SafeCertContextHandle
    {
        ...
        public static void DeleteKeyContainer(SafeCertContextHandle pCertContext)
        {
            ...
            uint cb = 0;
            bool containsPrivateKey = Interop.Crypt32.CertGetCertificateContextProperty(pCertContext, CertContextPropId.CERT_KEY_PROV_INFO_PROP_ID, (SafeLocalAllocHandle?)null, ref cb);
            ...
            unsafe
            {
                fixed (byte* pProvInfoAsBytes = provInfoAsBytes)
                {
                    ...
                    else
                    {
                        ...
                        _ = Interop.Crypt32.CryptAcquireContextA(ref hProv, new string(pProvInfo->pwszContainerName), new string(pProvInfo->pwszProvName), (uint)pProvInfo->dwProvType, flags);
